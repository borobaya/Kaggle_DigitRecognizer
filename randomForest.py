...
#param_grid = [
#    {'C': [1e2, 1e3, 1e4, 1e5], 'gamma': [1e-1, 1e-2, 1e-3, 1e-4], 'kernel': ['rbf']},
#    ]
#print "Performing grid search..."
#clf = GridSearchCV(   , param_grid, n_jobs=-1);
#clf.fit(X[:5000], y[:5000]); # Use a subsample of the data to find the best parameters
...
print "Saving test data classifications into "+path_out_randomForest+"..."
Zout = pd.DataFrame(Ztest)
Zout.index += 1
Zout.to_csv(path_out_randomForest, sep=',', encoding='utf-8', index_label=["ImageId"],\
    header=["Label"])
